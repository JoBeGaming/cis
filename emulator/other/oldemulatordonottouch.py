...
def update_tabs_on_screen():
    ...
    if current_tab == "CPU_code":
        ...
        CPU_code_content = tk.Text(CPU_frame, bg=dark_gray, fg=white, font=consolas, bd=0, insertbackground=white)
        ...
        CPU_code_content.tag_configure("error", underline=True, underlinefg="#F14C4C")
        ...
        # Add the titles before calling update_memory so they don't get created multiple times
        ...
        # Since RAM is too big, we make it scrollable
        ...
        # Call Stack
        callstack_title = tk.Label(CPU_frame, text="Call Stack", bg=black, fg=white, font=consolas)
        callstack_title.pack(side="top", fill="x")

        callstack_frame = tk.Frame(CPU_frame, bg=brown)
        callstack_frame.pack(side="top", fill="x")
